var g={canvas:0,c:0,title:"Games",col:{bg:pageColors.dark2,sub:pageColors.dark1}},p={canvas:0,c:0,title:"Projects",codeLines:[],indentSize:50,lineHeight:20,cindent:0,maxCindent:7,lineBottom:0,lineSep:1.5,indentPerc:0.07,col:{bg:pageColors.dark1,text:pageColors.light1,breakChance:0.2,previous:0,chances:{comment:0.08,color:0.03}}},coin={x:500,y:200,size:20},scrollsp=4,textMoveSp=0.3,mouse={x:0,y:0},hover="",clearbox={width:0,height:0},canvasesHover=false,format={slant:0,padding:0,slantW:0,fps:30,messageSp:3},font={size:70,font:"Sans-serif",color:pageColors.text},particles=[];
/*Todo: make mouse collide with diagonal line properly*/
// Main Functions
...
function main() {
	...
	draw(true);
	for(var i=0;i<p.codeLines.length;i++)p.codeLines[i].update();
...
function draw(clear) {
	if(clear) {
		...
		var dPadd=format.slantW*.6;
			format.padding=format.slantW*.2;
		// clear edge
		g.c.fillStyle="white";
		g.c.rotate(format.slant);
			var sx=Math.cos(-format.slant)*g.canvas.width,
				sy=Math.sin(-format.slant)*g.canvas.width;
			g.c.clearRect(sx,sy,g.canvas.width*.5,g.canvas.height*1.5);
		g.c.rotate(-format.slant);
		// project canvas draw
		...
		// p title
		outlinedText(p.c,p.title,dPadd*.4,p.canvas.height-format.padding/3,pageColors.dark1,"bottom","left");
		// g title
		outlinedText(g.c,g.title,g.canvas.width-dPadd*.4,format.padding/3,pageColors.dark1,"top","right");
	}
}
// Objects
function CodeLine(indent,length,y,color) {
	this.indent=indent;this.length=length;this.y=y;this.color=color;
...
function CoinParticle(x, y) {
	this.x = x;this.y = y;this.an = Math.random()*Math.PI*2;this.sp = sg.randomRange(3, 9);this.rad = sg.randomRange(3, 10);
	this.update=function() {
		if(hover=="g"){
			this.rad-=this.sp*.1;
...
function resize(){
	...
	//scale box
	clearbox.width=c;clearbox.height=a;format.slantW=c;font.size=p.canvas.width/8;
	// resize code things
	p.indentSize=p.canvas.width*p.indentPerc;p.lineHeight=p.canvas.height/35;
...
function makeLines() {
	...
	var minTab=3,tab=minTab,lineCount=(p.canvas.height/p.lineHeight)*5,minWid=0.03,col;
	for(var i=0;i<lineCount;i++){
		...
		if(Math.random()<p.col.chances.color)col=sg.choose(pageColors.red,pageColors.blue,pageColors.orange,pageColors.yellow);
		p.codeLines.push(new CodeLine(tab,Math.max(Math.random(),minWid),format.padding,col));
		
	}
	p.lineBottom=p.codeLines[0].y;
}
function outlinedText(c,string,x,y,outlineCol,baseline,align){c.fillStyle=font.color;c.font=font.size+"px "+font.font;c.textBaseline=baseline;c.textAlign=align;c.lineWidth=10;c.strokeStyle=outlineCol;c.strokeText(string,x,y);c.fillText(string,x,y);}